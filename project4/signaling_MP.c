...
int main (int argc, char *argv[]){
 ...
 // printf("INIT %d %d %d %d\n", shared_ptr->sigusr1_sent_counter, shared_ptr->sigusr2_sent_counter, shared_ptr->sigusr1_recieved_counter, shared_ptr->sigusr2_recieved_counter);   
  ...
  //printf("parent pid %d\n", getpid());
  puts("currently running ...  (multi-processes)");
  ...
  while(1){
    ...
    // if((shared_ptr->sigusr1_recieved_counter) + (shared_ptr->sigusr2_recieved_counter) >= 100000)
    //   break; 
  }

  puts("SEE p_data.LOG for REPORTS\n");
